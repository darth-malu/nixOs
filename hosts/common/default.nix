...
boot = {
  ...
  # initrd.kernelModules = if config.networking.hostName == "carthage" then [  "dm-snapshot" ] else [];
  initrd.availableKernelModules = if config.networking.hostName == "carthage" then
  ...
  kernelModules = if config.networking.hostName == "tangier" then
                  [ "kvm-intel"] else
                  [ "kvm-amd" "wl" ];
...
# dhcpcd.enable = true; # true:: # NOTE not sure if needed with networkmnager on
# useDHCP =  false;
