...
}:
{
  ...
  nix.settings = {
    ...
    allowed-users = [
      # Allowed to connect with the nix daemon. All users with *:: Trusted users always allowed to connect
      "@wheel"
      "malu"
      "remotebuild"
      "@darth"
  ...
  # NOTE: distributed builds (buildMachines, distributedBuilds, builders-use-substitutes)
  # are configured in ./users/remote-builder.nix to avoid duplication and IP conflicts.
