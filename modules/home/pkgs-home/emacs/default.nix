...

{
  programs.emacs = {
    ...
    '';
    extraPackages = epkgs:
      with epkgs; [ # nix-env -f '<nixpkgs>' -qaP -A emacsPackages
  ...
  services.emacs = {
    ...
    startWithUserSession =
      true; # whether to launch Emacs servicee with the systemd session
    #package = emacs;
    # defaultEditor = true;
    socketActivation.enable =
      true; # Whether to enable systemd socket activation for the Emacs service.
    client = {
