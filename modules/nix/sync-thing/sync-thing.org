...
* files to ignore
#+begin_src text :tangle ~/Shibuya/.stdignore
.git
#+end_src
* start
...
+ user - The user to run Syncthing as. By default, a user named syncthing will be created whose home directory is dataDir.
+ dataDir > The path where synchronised directories will exist.
...
add this if needed:
essential if running multi user setup...otherwise use openDefaultPorts
#+begin_src nix
